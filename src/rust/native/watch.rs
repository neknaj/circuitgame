...
pub async fn main(
    ...
    match &server_msg {
        ...
        Err(v) => {
            use colored::*;
            if !(watch | run_vm) {
                let _ = super::common::process_input(
                    &input_path,
                    output_modules_pattern,
                    output_path,
                    doc_output_path,
                );
...
async fn watch_file(
    ...
    while let Some(event) = watcher_rx.recv().await {
        ...
        let message = format!(
            "File change detected - Path: {}, Event: {:?}",
            path_str, event.kind
        );
...
async fn start_websocket_server(
    ...
    Ok(format!(
        ...
        format!(
            "\x1b]8;;{}\x1b\\{}\x1b]8;;\x1b\\",
            format!(
                "https://neknaj.github.io/circuitgame/?socket=ws://localhost:{}",
                port
            ),
...
async fn ncg_tool(
    ...
    output_path: Vec<String>,
    doc_output_path: Option<String>,
    server_msg: Result<String, String>,
    run_vm: bool,
    watch: bool,
    server: bool,
) {
    ...
    loop {
        ...
        let binaries = super::common::process_input(
            &input_path,
            output_modules_pattern.clone(),
            output_path.clone(),
            doc_output_path.clone(),
        );
