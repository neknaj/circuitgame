...
pub fn serialize(products: IntermediateProducts, module: &str) -> Result<Vec<u32>, String> {
    ...
    let expanded_module = match products.expanded_modules.get(module) {
        Some(v) => v.clone(),
        None => {
            return Err(format!("An undefined module was specified: {}", module));
        }
    };
...
pub fn intermediate_products(input: &str) -> types::IntermediateProducts {
    ...
    products.module_dependency_sorted =
        match sort_dependency(&products.module_dependency, &products.module_type_list) {
            Ok(res) => {
                products.warns.extend(res.1);
                res.0
            }
            Err(res) => {
                products.errors.extend(res.0);
                products.warns.extend(res.1);
                return products;
            }
        };
    // 6, 依存関係の先端から順にモジュールを展開 (全てのmoduleがnorのみで構成される)
    products.expanded_modules =
