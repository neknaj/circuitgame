...
pub fn module_expansion(
    ast: &File,
    modules: &Vec<String>,
) -> Result<HashMap<String, CompiledModule>, Vec<String>> {
    ...
    for module_name in modules.iter().rev() {
            ...
            expanded_modules.insert(
                ...
                CompiledModule {
                    ...
                    gates_sequential: vec![(
                        CompiledGateInput::Input(0),
                        CompiledGateInput::Input(1),
                    )],
        ...
        let module = match ast.components.iter().find_map(|component| {
            ...
            Some(v) => v,
            None => {
                errors.push(format!("Undefined module used: {}", module_name));
                continue;
            }
        };
        // 各ゲートのpointerを計算
        ...
        for gate in module.gates.clone() {
            let expanding_gate = match expanded_modules.get(&gate.module_name) {
                Some(v) => v.clone(),
                None => {
                    errors.push(format!("Undefined gate used: {}", gate.module_name));
                    continue;
                }
            };
        ...
        for gate in module.gates.clone() {
            ...
            for output in expanding_gate.outputs.clone() {
                ...
                output_map.insert(
                    output_name,
                    CompiledGateInput::NorGate(output + gates_pointer[gate_index as usize]),
                );
                output_index += 1;
            }
            gate_index += 1;
        ...
        for gate in module.gates.clone() {
            ...
            for egate in expanding_gate.gates_sequential.clone() {
                let input0 = match egate.0 {
                    CompiledGateInput::NorGate(n) => {
                        CompiledGateInput::NorGate(n + gates_pointer[gate_index as usize])
                    }
                    CompiledGateInput::Input(n) => match output_map.get(&gate.inputs[n as usize]) {
                        Some(v) => v.clone(),
                        None => {
                            errors
                                .push(format!("Undefined gate used: {}", gate.inputs[n as usize]));
                            continue;
                        }
                    },
                };
        ...
        for output in module.outputs.clone() {
            let output_solved = match output_map.get(&output) {
                Some(v) => v.clone(),
                None => {
                    errors.push(format!("Undefined gate used in output: {}", output));
                    continue;
                }
            };
