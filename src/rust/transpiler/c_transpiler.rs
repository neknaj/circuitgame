...
pub fn transpile(module: Module, header: bool) -> Result<String, String> {
    if !module.func {
        return Err(format!(
            "C transpiler does not support non-func module: {}",
            module.name
        ));
    }
    if module.gates_symmetry.len() > 0 {
        return Err(format!(
            "C transpiler does not support symmetry module: {}",
            module.name
        ));
    }
    // ヘッダーを作る
    ...
    let out_func_head = format!(
        ...
        (0..module.inputs as usize)
            .map(|i| format!("int b{}", i + module.gates_sequential.len()))
            .collect::<Vec<String>>()
            .join(", "),
    );
    let out_func_gates = module
        ...
        .iter()
        .enumerate()
        .map(|(index, value)| format!("    int b{} = !( b{} | b{} );", index, value.0, value.1))
        .collect::<Vec<String>>()
        .join("\n");
