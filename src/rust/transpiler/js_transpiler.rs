...
pub fn transpile(module: Module) -> Result<String, String> {
    if module.gates_symmetry.len() > 0 {
        return Err(format!(
            "JS transpiler does not support symmetry module: {}",
            module.name
        ));
    }
    if !module.func {
        return Err(format!(
            "JS transpiler does not support non-func module: {}",
            module.name
        ));
    } else {
        ...
        let out_func_head = format!(
            ...
            (0..module.inputs as usize)
                .map(|i| format!("b{}", i + module.gates_sequential.len()))
                .collect::<Vec<String>>()
                .join(", "),
        );
        let out_func_gates = module
            ...
            .enumerate()
            .map(|(index, value)| {
                format!("    const b{} = !( b{} || b{} );", index, value.0, value.1)
            })
            .collect::<Vec<String>>()
            .join("\n");
        ...
        let out_code = format!(
            "{}\n\n{}\n\n{}",
            "// Generated by Neknaj Circuit Game", out_const, out_func,
        );
        Ok(out_code)
    }
}
